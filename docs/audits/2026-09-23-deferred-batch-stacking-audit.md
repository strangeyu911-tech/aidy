...
以上**只改源码 + 测试**。运行中的 Aidy 仍是 `2026-09-22 21:00` 的产物，**同时不含 `6c8a72e` 与本次改动**。要生效需一次完整打包 + 重启。

**注意两者的耦合**：`6c8a72e` 让「临时失败」开始入队，本次改动决定入队后**多久失效**。二者应**同批部署** —— 只上 `6c8a72e` 会短暂出现「跨时段消息堆叠」（即本审计初版假设的症状），只上本次改动则等于时效窗口无货可管。
...
**两件事值得你马上知道**：
1. **修复已经写好了（`6c8a72e`，今天 16:18），但还没打包部署** —— 运行实例不含它。要生效必须重新打包并重启 Aidy。
2. **上线它之前，最好先处理「补发拼接无时间语义」** —— 因为修复后的行为会从「静默丢弃」变成「入队等下次上线一起补发」，那才会真正出现「8点了 / 12点多了」被拼成一大段发出来的观感（这正是我初版误判的症状，它今天还没发生，但**修复部署后就会具备发生条件**）。

> 本文件为只读审计，未修改任何源码、未重新打包、未清理运行时数据。所有结论均附可复现命令（§7）。
