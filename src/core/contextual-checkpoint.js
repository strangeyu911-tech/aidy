...
function inferContextualCheckpoint(text, { now = new Date(), durations = { meal: 30, shower: 30 }, quietHours = DEFAULT_QUIET_HOURS } = {}) {
  ...
  // This follow-up was inferred, not requested, so it does not override the
  // sleep window. Say so instead of promising a time Aidy will not keep.
  const deferredPastQuietHours = isWithinQuietHours(dueAt, "Asia/Shanghai", quietHours);
  return {
    ...
    announcement: deferredPastQuietHours
      ? "好，你先去吧，不过接下来是你的静默时段，我明早再找你。"
      : buildAnnouncement(minutes),
